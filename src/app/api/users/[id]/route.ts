...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  const session = await getSession();
  if (!session || session.role !== "ADMIN") {
    return NextResponse.json({ error: "Μόνο ο διαχειριστής μπορεί να διαγράψει χρήστη." }, { status: 403 });
  }
  const admin = await prisma.user.findUnique({ where: { id: session.sub } });
  if (!admin?.finalApprover) {
    return NextResponse.json({ error: "Μόνο διαχειριστής τελικής έγκρισης μπορεί να διαγράψει χρήστη." }, { status: 403 });
  }
  if (params.id === session.sub) {
    return NextResponse.json({ error: "Δεν μπορείς να διαγράψεις τον δικό σου λογαριασμό." }, { status: 400 });
  }

  const target = await prisma.user.findUnique({ where: { id: params.id } });
  if (!target) return NextResponse.json({ error: "Δεν βρέθηκε χρήστης." }, { status: 404 });

  // Διαγράφει πρώτα όλο το σχετικό ιστορικό, ώστε να μην μπλοκάρεται η διαγραφή από συνδέσεις στη βάση
  await prisma.$transaction([
    prisma.leaveRequest.deleteMany({ where: { userId: params.id } }),
    prisma.balanceAdjustment.deleteMany({ where: { userId: params.id } }),
    prisma.shiftSwap.deleteMany({ where: { OR: [{ requesterId: params.id }, { colleagueId: params.id }] } }),
